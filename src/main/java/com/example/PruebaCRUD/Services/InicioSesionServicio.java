...
public class InicioSesionServicio {
    ...
    public InicioSesionServicio(UsuarioRepositorio usuarioRepository) {
     ...
     */
    public InicioSesionRespuestaDTO inicioSesion(String username, String password) {
        System.out.println("EL USUARIO ES " + username);
        List<Object[]> results = usuarioRepository.callLoginFunction(username, password);

        if (results == null || results.isEmpty()) {
            return new InicioSesionRespuestaDTO(0, "Error inesperado.");
        }
        ...
        return new InicioSesionRespuestaDTO(username, error_code, message, role, cargos);
    }
}
